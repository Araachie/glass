...
def parse_args() -> ArgsNamespace:
    ...
    parser.add_argument("--step", type=int, default=None, help="Step to evaluate the model at.")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if args.num_gpus == 1:
            evaluate(rank=0, args=args, temp_dir=temp_dir)
        else:
            torch.multiprocessing.spawn(fn=evaluate, args=(args, temp_dir), nprocs=args.num_gpus)
